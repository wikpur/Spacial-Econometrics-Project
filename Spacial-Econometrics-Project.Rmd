...
county_names |> subset(subset = vpom == "22") -> pom00
```
...
```{r}
pom$TERYT <- sub("_", "", pom$TERYT)
any(is.na(match(pom$TERYT, pom00$TERYT)))


## ----pl_m3, size="footnotesize"-----------------------------------------------
pom |> merge(pom0, by = "TERYT") -> pom4 
pom4
```
...
names(ssusers)
pom1 |> merge(ssusers, by.x = "TERYT",
 by.y = "Code") -> pom4
pom4
```
```{r}
library(mapview)
mapview(pom4, zcol="social_services_users")
```
...
pom5 <- st_intersection(pom4, pom_gisco_tm1)
```
```{r}
pom4 |> st_geometry_type() |> table() -> tab5; tab5[tab5>0]
```

```{r}
pom5 |> st_geometry_type() |> table() -> tab6; tab6[tab6>0]
```

```{r}
pom5 |> st_cast("MULTIPOLYGON") -> pom5
```

```{r}
mapview(pom5, zcol="social_services_users")
```

```{r}
plot(density(pom5$social_services_users))
```
```{r}
mapsf::mf_map(pom5, var="social_services_users", type="choro", breaks="geom",
 nbreaks=7)
```
### III. 
