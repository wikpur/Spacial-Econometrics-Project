...
library(sp)
library(spdep)
(pom19 |> poly2nb(queen=FALSE,
 row.names=pom19$TERYT) -> pom_rook_nb)
```

```{r}
(pom19 |> poly2nb(queen=TRUE,
 row.names=pom19$TERYT) -> pom_queen_nb)
```

```{r}
(pom19 |> poly2nb(queen=FALSE, row.names=pom19$TERYT,
 snap=0.00001) -> pom_rook_nb)
(pom19 |> poly2nb(queen=TRUE, row.names=pom19$TERYT,
 snap=0.00001) -> pom_queen_nb)
```

```{r}
geom <- st_geometry(pom19) 
plot(geom, border="grey")
plot(pom_queen_nb, geom, add=TRUE)
```
```{r}
(geom |> st_point_on_surface() |> knearneigh(k=5) |>
 knn2nb(row.names=pom19$TERYT) -> pom_1_nb)
```
...
moran_test <- moran.test(wpom2$crimes, listw = lw, randomisation = TRUE, alternative = "two.sided")
print(moran_test)
...
lm_null <- lm(crimes ~ 1, data = wpom2)
...
```{r}
borders_get(TERYT = pom0$TERYT) -> wpom
ID <- as.integer("0012345") 
str(ID)
formatC(ID, format="d", width=7, flag="0")
dim(wpom)
...
```{r}
form_pre <- wpom2$crimes ~ wpom2$popdens + wpom2$avgsal +
 wpom2$unemp + wpom2$ssusers
lm_obj_pre <- lm(form_pre, data= wpom2)
...
library(spatialreg)
SLX_obj <- lmSLX(form_pre, data=wpom2, listw=lw,
 Durbin=update(form_pre, ~ . - Metrop))
...
library(spatialreg)
car1 <- spautolm(form_pre, data=wpom2, listw=lwB, family="CAR")
summary(car1, Nagelkerke=TRUE)
```
```{r}
wpom2$ranef_ml <- car1$fit$signal_stochastic
```
...
car2 <- hglm(fixed=form_pre, random= ~ 1|TERYT, data=wpom2, family = gaussian(), rand.family=CAR(D=W))
car2
```
```{r}
wpom2$ranef_hglm <- c(unname(car2$ranef))
```
...
wpom2$TERYT <- factor(wpom2$TERYT)
car3 <- gam(update(form_pre, . ~ . + s(TERYT, bs="mrf", xt=list(nb=pom_queen_nb))), data=wpom2)
summary(car3)

pom_queen_nb
```
```{r}
wpom2$ranef_mrf <- unname(predict(car3, type="terms")[, "s(TERYT)"])
```
...
```
```{r}
cor(st_drop_geometry(wpom2)[, c("ranef_ml", "ranef_hglm", "ranef_mrf")])
```
```{r}
...
```{r}
tm_shape(wpom2) + tm_fill(c("ranef_ml", "ranef_hglm", "ranef_mrf"), title="random effect", n=9, midpoint=0, style="fisher") + tm_borders() + tm_facets(free.scales=FALSE) + tm_layout(panel.labels=c("ML", "HGLM", "MRF"))
```
##SIMULTANEOUS AUTOREGRESSIVE MODELS

```{r}
sar1B <- spautolm(form_pre, data=wpom2, listw=lwB, family="SAR")
...
```
```{r}
sar1 <- spautolm(form_pre, data=wpom2, listw=lw, family="SAR", control=list(pre_eig=e))
summary(sar1, Nagelkerke=TRUE)
```
```{r}
SEM_pre <- errorsarlm(form_pre, data=wpom2, listw=lw, control=list(pre_eig=e))
