...
try(st_write(pom19, fn))
```

```{r}
lf <- list.files(pattern="pom19.gpkg")
if (length(lf) > 0L) tull <- file.remove(lf)
```

```{r}
str(pom19$NUTS_ID)
pom19$NUTS_ID <- 1:nrow(pom19)
st_write(pom19, dsn=fn)
```

```{r}
pom19a <- st_read(dsn=fn)
```
...
moran_test <- moran.test(pom18$Crimes, listw = lw, randomisation = TRUE, alternative = "two.sided")
print(moran_test)
...
lm_null <- lm(Crimes ~ 1, data = pom18)
